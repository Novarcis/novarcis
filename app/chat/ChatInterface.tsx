...
const MessageBubble = memo(({ message }: { message: Message }) => {
  return (
    ...
    >
      <div
        className={`max-w-[85%] md:max-w-[75%] ${message.role === "user" ? "order-1" : "flex items-start gap-3"
          }`}
      >
...
export default function ChatInterface() {
  ...
  return (
    <div className="w-full h-dvh flex flex-col lg:flex-row relative overflow-hidden bg-[#060810]">

      <Link
        href="/"
        className="absolute top-4 left-4 z-50 flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 rounded-full bg-[#0D1117]/60 backdrop-blur-md border border-[#1a1f2e] text-[#6B7A99] hover:text-[#00F5FF] hover:border-[#00F5FF]/40 transition-all group"
      >
        <ArrowLeft className="w-4 h-4 transition-transform group-hover:-translate-x-1" />
        <span className="hidden sm:inline text-xs font-mono uppercase tracking-wider">Volver</span>
      </Link>

      <div className="hidden lg:flex lg:w-1/2 h-full items-center justify-center bg-linear-to-br from-[#080A0F] to-[#0D1117] relative">
      ...
      <div className="flex-1 lg:w-1/2 flex flex-col h-full min-h-0 bg-[#060810] border-l border-[#1a1f2e]">

        <div className="shrink-0 flex items-center gap-3 px-6 py-4 border-b border-[#1a1f2e] pt-16 lg:pt-4">
          <div className="lg:hidden">
            <LogoEntity state={logoState} size="sm" />
          </div>
          <div>
            <h4 className="font-semibold text-[#E8EDF5]">Novarcis IA</h4>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 p-4 md:p-6 space-y-4 scrollbar-thin scrollbar-thumb-[#1a1f2e] scrollbar-track-transparent">
        ...
        <div className="shrink-0 p-4 border-t border-[#1a1f2e] bg-[#060810]">
          <div className="flex items-center gap-3 bg-[#0D1117] border border-[#1a1f2e] rounded-lg px-4 py-3 focus-within:border-[#00F5FF]/50 transition-colors">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-sm text-[#E8EDF5] outline-none font-mono caret-[#00F5FF] placeholder:text-[#6B7A99]"
            />

            <button
              onClick={sendMessage}
              disabled={!input.trim() || isLoading}
              className="shrink-0 p-2 rounded-md bg-[#00F5FF]/10 hover:bg-[#00F5FF]/20 text-[#00F5FF] disabled:opacity-30 transition-all"
              aria-label="Enviar"
            >
              <Send size={16} className="transform translate-x-0.5 -translate-y-0.5" />
            </button>
          </div>

          <p className="text-[10px] text-[#6B7A99] mt-2 text-center font-mono uppercase">
